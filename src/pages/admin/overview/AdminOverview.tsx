...
import {
  ...
  recentStaffUsers,
  staffRoleCounts,
} from '@/pages/admin/overview/overview.utils'
import './overview.css'
...
const AdminOverview = () => {
  ...
  const tasks = useMemo(
    () => adminSetupTasks(metrics, { includeUserTasks: manageUsers }),
    [metrics, manageUsers]
  )
          ...
          {roles.length === 0 ? (
            <div className="p-4 flex-1 flex items-center justify-center">
              <EmptyState
                icon="groups"
                title="No staff yet"
                description="Add users to assign hiring roles."
              />
            </div>
          ) : (
        ...
        {recentTeam.length === 0 ? (
          <div className="p-6">
            <EmptyState
              ...
              description="Add recruiters, HR, and interviewers from User management."
            />
          </div>
        ) : (
