...
import { userRoleLabel } from '@/lib/selectOptions'
import { mergeInterviewPanelLevels } from '@/lib/interviewPanelLevels'
...
export function adminSetupTasks(
  metrics: AdminSetupMetrics,
  options?: { includeUserTasks?: boolean }
): AdminSetupTask[] {
  const tasks: AdminSetupTask[] = []
  const includeUserTasks = options?.includeUserTasks ?? false
  ...
  if (includeUserTasks && metrics.disabledStaff > 0) {
    tasks.push({
      id: 'disabled',
  ...
  if (includeUserTasks && metrics.staffTotal <= 1) {
    tasks.push({
      id: 'add-users',
      title: 'Add your team',
