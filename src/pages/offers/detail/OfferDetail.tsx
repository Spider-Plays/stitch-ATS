...
import { FileText, Check, RotateCw, Lock, DollarSign, Send, Download, History, Clock, X, Trash2 } from 'lucide-react'
...
const OfferDetail = () => {
    ...
    const isRestricted = !['SUPER_ADMIN', 'ADMIN', 'HR_HEAD', 'HR_MANAGER'].includes(user?.role || '')
    ...
    const deleteMutation = useMutation({
        ...
        onError: (err: unknown) => {
            const msg = err instanceof ApiError ? err.message : 'Failed to delete offer'
            addToast(msg, 'error')
        },
    })
    ...
    const handleDelete = async () => {
        const ok = await confirm({
            title: 'Delete offer',
            message: `Permanently delete this offer for ${candidate?.name || 'this candidate'}? This cannot be undone.`,
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="flex flex-wrap gap-3">
                    ...
                    {offer.status === 'APPROVAL_PENDING' && (
                        ['SUPER_ADMIN', 'ADMIN', 'HR_HEAD'].includes(user?.role || '') ? (
                            <>
                                <button
                    ...
                    {isAdmin && (
                        <button
                            ...
                            className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-red-600 hover:bg-red-700 text-white text-sm font-bold disabled:opacity-50 transition-colors"
                        >
                            <Trash2 size={18} />
