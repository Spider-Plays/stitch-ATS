...
import { insertPlanStage, removePlanStage, renamePlanStage } from '@/lib/interviewPlanEdit'
        ...
        <div className="min-w-0 flex-1 pr-6">
          ...
          {canEditPlan && onRename ? (
            <input
              ...
              disabled={saving}
              className="w-full mt-0.5 px-2 py-1 -mx-2 rounded-lg border border-transparent hover:border-primary/15 focus:border-primary/25 dark:hover:border-white/15 dark:focus:border-white/25 bg-transparent font-bold text-sm text-primary dark:text-white outline-none focus:ring-2 focus:ring-primary/20 dark:focus:ring-white/20 disabled:opacity-50"
...
export function CandidateInterviewPipeline({
  ...
  const handleRename = (stageId: string, name: string) => {
    ...
    const payload = renamePlanStage(plan, stageId, name)
    saveMutation.mutate(payload)
  }
      ...
      <div className="flex flex-wrap items-end justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-primary/50 dark:text-white/50 mt-0.5">
            ...
            {canEditPlan &&
              ' Click a round name to edit it, hover between rounds to add a stage, or use the trash icon to remove one.'}
          </p>
        </div>
